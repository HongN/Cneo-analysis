	...
	for vcf_line in vcf_file:
		if (re.search(comment_pattern, vcf_line)):
			...
			for sample in samples:
				...
				if (split_genotype[0] == '0' or split_genotype[0] == '0/0' or split_genotype[0] == '0|0') and (keep_all_ref):
					split_genotype[0],GQ_flag,AD_flag,DP_flag,Bi_flag = record.get_genotype(index=header.get_sample_index(sample),return_flags=True)
				elif (split_genotype[0] == '0' or split_genotype[0] == '0/0' or split_genotype[0] == '0|0') and (gq == '0') and (keep_GQ_0_refs):
					split_genotype[0],GQ_flag,AD_flag,DP_flag,Bi_flag = record.get_genotype(index=header.get_sample_index(sample),min_tot_dp=min_tot_DP,return_flags=True)
				elif (split_genotype[0] == '0' or split_genotype[0] == '0/0' or split_genotype[0] == '0|0'):
					split_genotype[0],GQ_flag,AD_flag,DP_flag,Bi_flag = record.get_genotype(index=header.get_sample_index(sample),min_gq=min_GQ,min_tot_dp=min_tot_DP,return_flags=True)
				else:
					split_genotype[0],GQ_flag,AD_flag,DP_flag,Bi_flag = record.get_genotype(index=header.get_sample_index(sample),min_gq=min_GQ,min_per_ad=min_AD,min_tot_dp=min_tot_DP,het_binom_p=het_binomial_p,return_flags=True)
					#split_genotype[0] = record.get_genotype(index=header.get_sample_index(sample),min_gq=min_GQ,min_per_ad=min_AD,min_tot_dp=min_tot_DP)
